...
function suggestTenantRealizedAnsibleProjectIds(filters, $list, tenantResource = null, ansibleProjectIds = null, relate=true, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        ...
        if(tenantResource != null) {
          $input.addEventListener('change', function(event) {
            patchTenantVals([{ name: 'fq', value: 'tenantResource:' + tenantResource }], { [(event.target.checked ? 'add' : 'remove') + 'AnsibleProjectIds']: o[inputVar] }
                ...
                , function(response, target) {
                  addGlow(target);
                  suggestTenantRealizedAnsibleProjectIds(filters, $list, tenantResource, o[inputVar], relate, target);
                }
                , function(response, target) { addError(target); }
            );
          });
