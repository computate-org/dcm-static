...
async function websocketAiTelemetryPlatform(success) {
  window.eventBus.onopen = function () {

    window.eventBus.registerHandler('websocketAiTelemetryPlatform', function (error, message) {
...
async function websocketAiTelemetryPlatformInner(apiRequest) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        if(vars.includes('modified'))
          inputModified = $response.querySelector('.AiTelemetryPlatform_Page_modified');
        if(vars.includes('archived'))
          inputArchived = $response.querySelector('.AiTelemetryPlatform_Page_archived');
        if(vars.includes('name'))
          inputName = $response.querySelector('.AiTelemetryPlatform_Page_name');
        if(vars.includes('description'))
          inputDescription = $response.querySelector('.AiTelemetryPlatform_Page_description');
        if(vars.includes('authorName'))
          inputAuthorName = $response.querySelector('.AiTelemetryPlatform_Page_authorName');
        if(vars.includes('authorUrl'))
          inputAuthorUrl = $response.querySelector('.AiTelemetryPlatform_Page_authorUrl');
        if(vars.includes('pageImageUri'))
          inputPageImageUri = $response.querySelector('.AiTelemetryPlatform_Page_pageImageUri');
        ...
        if(vars.includes('editPage'))
          inputEditPage = $response.querySelector('.AiTelemetryPlatform_Page_editPage');
        if(vars.includes('userPage'))
          inputUserPage = $response.querySelector('.AiTelemetryPlatform_Page_userPage');
        ...
        if(vars.includes('nextArticleIds'))
          inputNextArticleIds = $response.querySelector('.AiTelemetryPlatform_Page_nextArticleIds');
        if(vars.includes('nextArticles'))
          inputNextArticles = $response.querySelector('.AiTelemetryPlatform_Page_nextArticles');
        if(vars.includes('labelsString'))
          inputLabelsString = $response.querySelector('.AiTelemetryPlatform_Page_labelsString');
        if(vars.includes('labels'))
          inputLabels = $response.querySelector('.AiTelemetryPlatform_Page_labels');
        ...
        if(inputCreated) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_created'));
        }
        ...
        if(inputModified) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_modified'));
        }
        ...
        if(inputArchived) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_archived'));
        }
        ...
        if(inputName) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_name'));
        }
        ...
        if(inputDescription) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_description'));
        }
        ...
        if(inputAuthorName) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_authorName'));
        }
        ...
        if(inputAuthorUrl) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_authorUrl'));
        }
        ...
        if(inputPageImageUri) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_pageImageUri'));
        }
        ...
        if(inputPageId) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_pageId'));
        }
        ...
        if(inputObjectTitle) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_objectTitle'));
        }
        ...
        if(inputEditPage) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_editPage'));
        }
        ...
        if(inputUserPage) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_userPage'));
        }
        ...
        if(inputDownload) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_download'));
        }
        ...
        if(inputHubResource) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_hubResource'));
        }
        ...
        if(inputPageImageType) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_pageImageType'));
        }
        ...
        if(inputPageImageAlt) {
          ...
          addGlow(document.querySelector('.AiTelemetryPlatform_Page_pageImageAlt'));
        }
...
function pageGraphAiTelemetryPlatform(apiRequest) {
    ...
    if(json['facetCounts']) {
      ...
      if(facetCounts['facetPivot'] && facetCounts['facetRanges']) {
        ...
        if(range) {
          ...
          Plotly.react('htmBodyGraphAiTelemetryPlatformPage', data, layout);
        }
      }
    }
  }
...
function animateStats() {
  document.querySelector('#pageSearchVal-fqAiTelemetryPlatform_time').innerText = '';
  searchPage('AiTelemetryPlatform', function() {
    ...
    let animateInterval = window.setInterval(() => {
      ...
      document.querySelector('#fqAiTelemetryPlatform_time').value = x;
      document.querySelector('#fqAiTelemetryPlatform_time').onchange();
      searchPage('AiTelemetryPlatform');
    }, speedRate);
  });
}
...
async function searchAiTelemetryPlatform($formFilters, success, error) {
  var filters = searchAiTelemetryPlatformFilters($formFilters);
  if(success == null)
    success = function( data, textStatus, jQxhr ) {};
  if(error == null)
    error = function( jqXhr, target2 ) {};

  searchAiTelemetryPlatformVals(filters, target, success, error);
}

function searchAiTelemetryPlatformFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function searchAiTelemetryPlatformVals(filters, target, success, error) {

  fetch(
    '/en-us/api/ai-telemetry-platform?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
...
function suggestAiTelemetryPlatformObjectSuggest($formFilters, $list, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
  ...
  error = function( jqXhr, target2 ) {};
  searchAiTelemetryPlatformVals($formFilters, target, success, error);
}

// GET //

async function getAiTelemetryPlatform() {
  fetch(
    '/en-us/api/ai-telemetry-platform/' + pageId
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
    }).then(response => {
      if(response.ok) {
...
async function patchAiTelemetryPlatform($formFilters, $formValues, target, pageId, success, error) {
  var filters = patchAiTelemetryPlatformFilters($formFilters);
...
function patchAiTelemetryPlatformFilters($formFilters) {
  var filters = [];
  if($formFilters) {
...
function patchAiTelemetryPlatformVal(filters, v, val, target, success, error) {
  var vals = {};
  vals[v] = val;
  patchAiTelemetryPlatformVals(filters, vals, target, success, error);
}

function patchAiTelemetryPlatformVals(filters, vals, target, success, error) {
  fetch(
    '/en-us/api/ai-telemetry-platform?' + filters.map(function(m) { return m.name + '=' + encodeURIComponent(m.value) }).join('&')
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function postAiTelemetryPlatform($formValues, target, success, error) {
  var vals = {};
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
...
function postAiTelemetryPlatformVals(vals, target, success, error) {
  fetch(
    '/en-us/api/ai-telemetry-platform'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
...
async function deleteAiTelemetryPlatform(target, pageId, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
  ...
  fetch(
    '/en-us/api/ai-telemetry-platform/' + encodeURIComponent(pageId)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportAiTelemetryPlatform($formValues, target, pageId, success, error) {
  var json = $formValues.querySelector('.PUTImport_searchList')?.value;
  if(json != null && json !== '')
    putimportAiTelemetryPlatformVals(JSON.parse(json), target, success, error);
}

function putimportAiTelemetryPlatformVals(json, target, success, error) {
  fetch(
    '/en-us/api/ai-telemetry-platform-import'
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'PUT'
...
async function deletefilterAiTelemetryPlatform(target, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
