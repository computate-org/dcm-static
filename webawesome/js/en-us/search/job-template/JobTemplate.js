...
async function websocketJobTemplate(success) {
  window.eventBus.onopen = function () {
    ...
    window.eventBus.registerHandler('websocketJobTemplate', function (error, message) {
      ...
      $box.setAttribute('class', 'w3-quarter box-' + jobTemplateName + ' ');
      $box.setAttribute('id', 'box-' + jobTemplateName);
      $box.setAttribute('data-numPATCH', numPATCH);
      var $margin = document.createElement('div');
      $margin.setAttribute('class', 'w3-margin ');
      $margin.setAttribute('id', 'margin-' + jobTemplateName);
      var $card = document.createElement('div');
      $card.setAttribute('class', 'w3-card w3-white ');
      $card.setAttribute('id', 'card-' + jobTemplateName);
      var $header = document.createElement('div');
      $header.setAttribute('class', 'w3-container fa- ');
      $header.setAttribute('id', 'header-' + jobTemplateName);
      ...
      $x.setAttribute('onclick', 'document.querySelector("#card-' + jobTemplateName + '");');
      $x.classList.add("display-none");
      $x.setAttribute('id', 'x-' + jobTemplateName);
      var $body = document.createElement('div');
      $body.setAttribute('class', 'w3-container w3-padding ');
      $body.setAttribute('id', 'text-' + jobTemplateName);
      var $bar = document.createElement('div');
      $bar.setAttribute('class', 'w3-light-gray ');
      $bar.setAttribute('id', 'bar-' + jobTemplateName);
      var $progress = document.createElement('div');
      $progress.setAttribute('class', 'w3- ');
      $progress.setAttribute('style', 'height: 24px; width: ' + percent + '; ');
      $progress.setAttribute('id', 'progress-' + jobTemplateName);
      ...
      if(numPATCH < numFound) {
        var $old_box = document.querySelector('.box-' + jobTemplateName);
      } else {
        document.querySelector('.box-' + jobTemplateName)?.remove();
      }
      if(jobTemplateName) {
        if(success)
          success(json);
...
async function websocketJobTemplateInner(apiRequest) {
  var jobTemplateName = apiRequest['id'];
  var classes = apiRequest['classes'];
  var vars = apiRequest['vars'];
  var empty = apiRequest['empty'];

  if(jobTemplateName != null && vars.length > 0) {
    ...
    fetch(uri).then(response => {
      response.text().then(text => {
        ...
        jsWebsocketJobTemplate(jobTemplateName, vars, $response);
...
function suggestJobTemplateTenantResource(filters, $list, jobTemplateName = null, tenantResource = null, relate=true, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        ...
        var checked = val == null ? false : (Array.isArray(val) ? val.includes(jobTemplateName.toString()) : val == tenantResource);
        var $input = document.createElement('wa-checkbox');
        $input.setAttribute('id', 'GET_tenantResource_' + jobTemplateName + '_tenantResource_' + o['tenantResource']);
        ...
        if(jobTemplateName != null) {
          $input.addEventListener('change', function(event) {
            patchJobTemplateVals([{ name: 'fq', value: 'jobTemplateName:' + jobTemplateName }], { [(event.target.checked ? 'set' : 'remove') + 'TenantResource']: o['tenantResource'] }
                , target
                , function(response, target) {
                  addGlow(target);
                  suggestJobTemplateTenantResource(filters, $list, jobTemplateName, o['tenantResource'], relate, target);
                }
...
function suggestJobTemplateInventoryResource(filters, $list, jobTemplateName = null, inventoryResource = null, relate=true, target) {
  success = function( data, textStatus, jQxhr ) {
    if($list) {
      $list.innerHTML = '';
      data['list'].forEach((o, i) => {
        ...
        var checked = val == null ? false : (Array.isArray(val) ? val.includes(jobTemplateName.toString()) : val == inventoryResource);
        var $input = document.createElement('wa-checkbox');
        $input.setAttribute('id', 'GET_inventoryResource_' + jobTemplateName + '_inventoryResource_' + o['inventoryResource']);
        ...
        if(jobTemplateName != null) {
          $input.addEventListener('change', function(event) {
            patchJobTemplateVals([{ name: 'fq', value: 'jobTemplateName:' + jobTemplateName }], { [(event.target.checked ? 'set' : 'remove') + 'InventoryResource']: o['inventoryResource'] }
                , target
                , function(response, target) {
                  addGlow(target);
                  suggestJobTemplateInventoryResource(filters, $list, jobTemplateName, o['inventoryResource'], relate, target);
                }
                , function(response, target) { addError(target); }
            );
          });
...
async function getJobTemplate(pk) {
  fetch(
    '/en-us/api/job-template/' + jobTemplateName
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
    }).then(response => {
      if(response.ok) {
...
async function patchJobTemplate($formFilters, $formValues, target, jobTemplateName, success, error) {
  ...
  patchJobTemplateVals(jobTemplateName == null ? deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'jobTemplateName:' + jobTemplateName}], vals, target, success, error);
}
...
async function deleteJobTemplate(target, jobTemplateName, success, error) {
  if(success == null) {
    success = function( data, textStatus, jQxhr ) {
      addGlow(target, jqXhr);
      var url = data['editPage'];
  ...
  fetch(
    '/en-us/api/job-template/' + encodeURIComponent(jobTemplateName)
    , {
      headers: {'Content-Type':'application/json; charset=utf-8'}
      , method: 'DELETE'
    }).then(response => {
...
async function putimportJobTemplate($formValues, target, jobTemplateName, success, error) {
